...
- **Chat persistence.** After every turn, `/chat` and `/chat/stream` call `_persist_conversation()`, which pulls the full thread from the agent's checkpointed state (`agent.get_state(config)` for the stream), keeps just the user/assistant text (drops system + tool messages), and writes `chats/<thread_id>.json` (title from the first message, `created_at` preserved) so the React sidebar's `/api/chats` refetch shows it. (Earlier the endpoints never saved anything — `_save_chat` was defined but uncalled — so the chat list never updated.)
